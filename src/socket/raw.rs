...
use wire::{IpProtocol, IpRepr, IpVersion};
...
mod test {
    ...
    mod ipv4_locals {
        ...
        pub fn socket(
            rx_buffer: RawSocketBuffer<'static, 'static>,
        ...
        ($module:ident, $socket:path, $hdr:path, $packet:path, $payload:path) => {
            mod $module {
                ...
                fn test_send_dispatch() {
                    ...
                    assert_eq!(
                        socket.dispatch(&checksum_caps, |_| unreachable!()),
                        Err(Error::Exhausted)
                    );
                    ...
                    assert_eq!(
                        socket.dispatch(&checksum_caps, |(ip_repr, ip_payload)| {
                ...
                fn test_recv_truncated_slice() {
                    ...
                    assert_eq!(
                        socket.process(&$hdr, &$payload, &ChecksumCapabilities::default()),
                        Ok(())
                    );
                ...
                fn test_recv_truncated_packet() {
                    ...
                    assert!(socket.accepts(&$hdr));
                    assert_eq!(
                        socket.process(&$hdr, &buffer, &ChecksumCapabilities::default()),
                        Err(Error::Truncated)
                    );
                }
            }
    ...
    fn test_send_illegal() {
        ...
        #[cfg(feature = "proto-ipv4")]
        {
            ...
            assert_eq!(socket.dispatch(&checksum_caps, |_| unreachable!()), Ok(()));
