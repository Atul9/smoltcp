...
use wire::{IpAddress, IpEndpoint, IpProtocol, IpRepr, TcpControl, TcpRepr, TcpSeqNumber};
use {Error, Result};
...
impl Timer {
    ...
    fn set_keep_alive(&mut self) {
        match *self {
            ...
            } if keep_alive_at.is_none() => *keep_alive_at = Some(Instant::from_millis(0)),
            _ => (),
        }
    }
...
impl<'a> TcpSocket<'a> {
    ...
    pub(crate) fn process(
            ...
            (
                ...
                if ack_number < self.local_seq_no {
                    net_debug!(
                        ...
                        ack_number,
                        self.local_seq_no,
                        self.local_seq_no + unacknowledged
                    );
                    return Err(Error::Dropped);
                }
        ...
        self.remote_win_len =
            (repr.window_len as usize) << (self.remote_win_scale.unwrap_or(0) as usize);
    ...
    fn seq_to_transmit(&self) -> bool {
        let control;
        match self.state {
            State::SynSent | State::SynReceived => control = TcpControl::Syn,
            State::FinWait1 | State::LastAck => control = TcpControl::Fin,
        ...
        match self.state {
            ...
            State::Established | State::FinWait1 | State::CloseWait | State::LastAck => {
                ...
                if offset + repr.payload.len() == self.tx_buffer.len() {
                    match self.state {
                        State::FinWait1 | State::LastAck => repr.control = TcpControl::Fin,
...
mod test {
    ...
    fn test_set_hop_limit() {
        ...
        assert_eq!(
            s.dispatch(Instant::from_millis(0), &caps, |(ip_repr, _)| {
