...
impl<'a, 'b: 'a, 'c: 'a + 'b> Set<'a, 'b, 'c> {
        ...
        T: Into<Socket<'b, 'c>>,
    {
        fn put<'b, 'c>(
            index: usize,
            slot: &mut Option<Item<'b, 'c>>,
            mut socket: Socket<'b, 'c>,
        ) -> Handle {
        ...
        for (index, item) in self.sockets.iter_mut().enumerate() {
            ...
            }) = item
            {
                match socket {
                    ...
                    &mut Socket::Raw(_) => may_remove = true,
                    #[cfg(all(
                        feature = "socket-icmp",
                        any(feature = "proto-ipv4", feature = "proto-ipv6")
                    ))]
                    &mut Socket::Icmp(_) => may_remove = true,
                    #[cfg(feature = "socket-udp")]
                    &mut Socket::Udp(_) => may_remove = true,
                    #[cfg(feature = "socket-tcp")]
