...
#[cfg(feature = "proto-ipv4")]
use wire::{Icmpv4DstUnreachable, Icmpv4Packet, Icmpv4Repr};
#[cfg(feature = "proto-ipv6")]
use wire::{Icmpv6Packet, Icmpv6ParamProblem, Icmpv6Repr};
...
#[cfg(feature = "proto-ipv6")]
use wire::{Ipv6OptionFailureType, Ipv6OptionRepr};
#[cfg(feature = "proto-ipv6")]
use wire::{NdiscNeighborFlags, NdiscRepr};
#[cfg(feature = "socket-tcp")]
use wire::{TcpControl, TcpPacket, TcpRepr};
#[cfg(feature = "socket-udp")]
use wire::{UdpPacket, UdpRepr};
use {Error, Result};

use super::Routes;
use super::{NeighborAnswer, NeighborCache};
#[cfg(all(
    feature = "socket-icmp",
    any(feature = "proto-ipv4", feature = "proto-ipv6")
))]
...
use socket::{AnySocket, PollAt, Socket, SocketSet};
    ...
    DeviceT: for<'d> Device<'d>,
{
    ...
    fn socket_egress(&mut self, sockets: &mut SocketSet, timestamp: Instant) -> Result<bool> {
        ...
        for mut socket in sockets.iter_mut() {
            if !socket
                .meta_mut()
                .egress_permitted(|ip_addr| self.inner.has_neighbor(&ip_addr, timestamp))
            {
                continue;
            }
            ...
            let socket_result = match *socket {
                ...
                Socket::Raw(ref mut socket) => {
                    socket.dispatch(&caps.checksum, |response| respond!(Packet::Raw(response)))
                }
                #[cfg(all(
                    feature = "socket-icmp",
                    any(feature = "proto-ipv4", feature = "proto-ipv6")
                ))]
                Socket::Icmp(ref mut socket) => socket.dispatch(&caps, |response| match response {
                    #[cfg(feature = "proto-ipv4")]
            ...
            match (device_result, socket_result) {
                ...
                (Err(Error::Unaddressable), _) => {
                    ...
                    // neighboor.
                    socket.meta_mut().neighbor_missing(
                        timestamp,
...
impl<'b, 'c, 'e> InterfaceInner<'b, 'c, 'e> {
    ...
    fn process_ethernet<'frame, T: AsRef<[u8]>>(
        ...
        match eth_frame.ethertype() {
            ...
            #[cfg(feature = "proto-ipv6")]
            EthernetProtocol::Ipv6 => self.process_ipv6(sockets, timestamp, &eth_frame),
    ...
    fn process_ipv6<'frame, T: AsRef<[u8]>>(
        ...
        if eth_frame.src_addr().is_unicast() {
            ...
            if self.in_same_network(&ip_addr)
                && self
                    .neighbor_cache
                    .lookup_pure(&ip_addr, timestamp)
                    .is_none()
            {
                self.neighbor_cache
                    .fill(ip_addr, eth_frame.src_addr(), timestamp);
            }
        }
    ...
    fn process_nxt_hdr<'frame>(
        ...
        match nxt_hdr {
            ...
            #[cfg(feature = "socket-tcp")]
            IpProtocol::Tcp => self.process_tcp(sockets, timestamp, ipv6_repr.into(), ip_payload),
            ...
            #[cfg(feature = "socket-raw")]
            _ if handled_by_raw_socket => Ok(Packet::None),
    ...
    fn process_ipv4<'frame, T: AsRef<[u8]>>(
            ...
            && !self.has_multicast_group(ipv4_repr.dst_addr)
        {
            ...
            if !self.any_ip {
                ...
                .routes
                .lookup(&IpAddress::Ipv4(ipv4_repr.dst_addr), timestamp)
            {
        ...
        match ipv4_repr.protocol {
            ...
            #[cfg(feature = "proto-igmp")]
            IpProtocol::Igmp => self.process_igmp(timestamp, ipv4_repr, ip_payload),
    ...
    fn process_icmpv6<'frame>(
        ...
        for mut icmp_socket in _sockets.iter_mut().filter_map(IcmpSocket::downcast) {
            if !icmp_socket.accepts(&ip_repr, &icmp_repr.into(), &checksum_caps) {
                continue;
            }
    ...
    fn process_udp<'frame>(
        ...
        match ip_repr {
            #[cfg(feature = "proto-ipv4")]
            IpRepr::Ipv4(_) if handled_by_raw_socket => Ok(Packet::None),
            #[cfg(feature = "proto-ipv6")]
            IpRepr::Ipv6(_) if handled_by_raw_socket => Ok(Packet::None),
            #[cfg(feature = "proto-ipv4")]
        ...
        Tx: TxToken,
    {
        ...
        match packet {
            ...
            #[cfg(feature = "proto-igmp")]
            Packet::Igmp((ipv4_repr, igmp_repr)) => self.dispatch_ip(
                tx_token,
                timestamp,
                IpRepr::Ipv4(ipv4_repr),
                |_ip_repr, payload| {
...
mod test {
    ...
    use iface::{EthernetInterface, NeighborCache};
    use phy::{self, ChecksumCapabilities, Loopback};
    #[cfg(feature = "proto-igmp")]
    ...
    use wire::{EthernetAddress, EthernetFrame, EthernetProtocol};
    #[cfg(feature = "proto-ipv4")]
    use wire::{Icmpv4DstUnreachable, Icmpv4Repr};
    #[cfg(feature = "proto-ipv6")]
    use wire::{Icmpv6Packet, Icmpv6ParamProblem, Icmpv6Repr};
    #[cfg(feature = "proto-igmp")]
    use wire::{IgmpPacket, IgmpRepr, IgmpVersion};
    ...
    fn create_loopback<'a, 'b, 'c>() -> (
        EthernetInterface<'static, 'b, 'c, Loopback>,
    ...
    #[cfg(feature = "proto-igmp")]
    fn recv_all<'b>(
        iface: &mut EthernetInterface<'static, 'b, 'static, Loopback>,
        timestamp: Instant,
    ) -> Vec<Vec<u8>> {
    ...
    fn test_no_icmp_no_unicast() {
        ...
        assert_eq!(
            iface
                .inner
                .process_ipv4(&mut socket_set, Instant::from_millis(0), &frame),
            Ok(Packet::None)
        );
        #[cfg(feature = "proto-ipv6")]
        assert_eq!(
            iface
                .inner
                .process_ipv6(&mut socket_set, Instant::from_millis(0), &frame),
            Ok(Packet::None)
        );
    }
    ...
    fn test_icmp_error_port_unreachable() {
        ...
        assert_eq!(
            iface
                .inner
                .process_udp(&mut socket_set, ip_repr, false, data),
            Ok(expected_repr)
        );
    ...
    fn test_handle_udp_broadcast() {
        use socket::{UdpPacketMetadata, UdpSocket, UdpSocketBuffer};
        use wire::IpEndpoint;
        ...
        assert_eq!(
            iface
                .inner
                .process_udp(&mut socket_set, ip_repr, false, packet.into_inner()),
            Ok(Packet::None)
        );
    ...
    fn test_handle_ipv4_broadcast() {
        use wire::{Icmpv4Packet, Icmpv4Repr, Ipv4Packet};
    ...
    fn test_icmp_reply_size() {
        ...
        assert_eq!(
            iface
                .inner
                .process_udp(&mut socket_set, ip_repr.into(), false, payload),
    ...
    fn test_icmpv4_socket() {
        use socket::{IcmpEndpoint, IcmpPacketMetadata, IcmpSocket, IcmpSocketBuffer};
        ...
        assert_eq!(
            iface
                .inner
                .process_icmpv4(&mut socket_set, ip_repr, icmp_data),
    ...
    fn test_handle_igmp() {
        fn recv_igmp<'b>(
            mut iface: &mut EthernetInterface<'static, 'b, 'static, Loopback>,
            timestamp: Instant,
        ...
        const GENERAL_QUERY_BYTES: &[u8] = &[
            0x01, 0x00, 0x5e, 0x00, 0x00, 0x01, 0x0a, 0x14, 0x48, 0x01, 0x21, 0x01, 0x08, 0x00,
        ...
        for group in &groups {
            iface
                .leave_multicast_group(group.clone(), timestamp)
                .unwrap();
        }
    ...
    fn test_raw_socket_no_reply() {
        use socket::{RawPacketMetadata, RawSocket, RawSocketBuffer};
        ...
        let frame = {
            ...
            udp_repr.emit(
                &mut UdpPacket::new_unchecked(&mut frame.payload_mut()[ipv4_repr.buffer_len()..]),
                &src_addr.into(),
                &dst_addr.into(),
                &ChecksumCapabilities::default(),
            );
    ...
    fn test_raw_socket_with_udp_socket() {
        ...
        use wire::{IpEndpoint, IpVersion, Ipv4Packet, UdpPacket, UdpRepr};
